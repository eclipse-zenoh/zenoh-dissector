    ...
    static ref WIRESHARK_SOURCE_DIR: PathBuf = {
        env::var("WIRESHARK_SOURCE_DIR")
            ...
            .unwrap_or_else(|_| {
                ...
                {
                    // Build scripts run with cwd = CARGO_MANIFEST_DIR, so
                    // resolve to an absolute path here. The cmake crate's
                    // .out_dir() stores paths verbatim (no resolution); if it
                    // receives a relative path, cmake --build resolves that
                    // argument *again* relative to cmake's own working
                    // directory (which the cmake crate sets to the same build
                    // sub-dir), producing a double-nested nonexistent path
                    // like "wireshark-X.Y.Z/build/wireshark-X.Y.Z/build".
                    env::current_dir()
                        .expect("failed to get current directory")
                        .join(format!("wireshark-{}", *WIRESHARK_VERSION))
                }
            })
    };
...
fn link_wireshark() -> Result<()> {
    ...
    if build_from_source {
        // On Windows (MSVC multi-config generator) cmake places the built
        // libraries under run/<Config>/.  On Unix the single-config generator
        // places them directly under run/.
        #[cfg(target_os = "windows")]
        let lib_dir = {
            ...
            WIRESHARK_BUILD_DIR.join("run").join(build_config)
        };
        #[cfg(not(target_os = "windows"))]
        let lib_dir = WIRESHARK_BUILD_DIR.join("run");
        ...
        // On Windows verify the import libraries (.lib) exist.
        #[cfg(target_os = "windows")]
        {
...
fn generate_bindings() -> Result<()> {
    ...
    match pkg_config::probe_library("wireshark") {
        ...
        Err(_) => {
            // Mirror the same platform-specific layout used in link_wireshark().
            #[cfg(target_os = "windows")]
            let lib_dir = {
                let build_config = if cfg!(debug_assertions) {
                    "Debug"
            ...
            builder = builder
                ...
                // ws_version.h and config.h are generated by cmake into the
                // build directory, so it must also be on the include path.
                .clang_arg(format!("-I{}", WIRESHARK_BUILD_DIR.to_string_lossy()));

            // glib headers are required by wireshark.h on all platforms.
            #[cfg(target_os = "windows")]
            {
